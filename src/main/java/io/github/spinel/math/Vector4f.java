 ...
 */
public class Vector4f extends VectorNf<Vector4f> {
    ...
    public float norm() {
        return (float) Math.sqrt(
                content[0] * content[0] + content[1] * content[1] + content[2] * content[2] + content[3] * content[3]);
    }

    @Override
    public Vector4f normalize() {
        float length = norm();
        return divide(new Vector4f(length, length, length, length));
    }
